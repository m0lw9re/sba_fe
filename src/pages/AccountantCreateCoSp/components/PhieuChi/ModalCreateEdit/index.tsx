...
import "./style.scss";
import ModalCreateEditTable from "./ModalCreateEditTable";
...
const AddAndEditModal: React.FC<Props> = ({ isOpen, onClose, action }) => {
  const [form] = useForm();
    ...
    >
      <Space direction="vertical" size={"small"} className="space-assignment">
        <Row
          ...
          align={"middle"}
          className="modalAccountantcrsp-header"
          style={{ width: "100%" }}
        >
          <CardTitleCustomUpdate
            title={action === "add" ? "Thêm phiếu chi mới" : "Sửa phiếu chi"}
            color="#fff"
          />
          <Button
            shape="circle"
