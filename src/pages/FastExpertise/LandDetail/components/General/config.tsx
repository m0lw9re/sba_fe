...
const checkTitle = (record: any) => {
  if (
    record?.type == "Tổng giá trị đất" ||
    record?.type == "Tổng giá trị công trình xây dựng" ||
    record?.type == "Tổng giá trị tài sản"
  ) {
    return true;
  } else {
    return false;
...
const columns: ColumnsType<any> = [
  ...
  },
  {
    ...
    align: "right",
    render(value, record, index) {
      return <>{value !== null ? `${formatToCurrencyType(value)} đ` : ''}</>;
    },
  },
  {
    title: "Giá trị",
...
const dataSource1Init: any[] = [
  {
    key: 1,
    type: "Đất công nhận phù hợp quy hoạch",
    area: 0,
    price: 0,
    value: 0,
  },
  {
    key: 2,
    type: "Đất công nhận không phù hợp quy hoạch",
    area: 0,
    price: 0,
    value: 0,
  },
  {type: "Tổng giá trị đất", area: 0, price: 0, value: 0},
  ,
];
const dataSource3Init = [
  {
    type: "Tổng giá trị tài sản",
    area: 0,
    price: 0,
    value: 0,
  },
