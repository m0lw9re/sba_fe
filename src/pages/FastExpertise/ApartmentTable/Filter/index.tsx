...
const Filter = forwardRef(({ setFilters }: Props, ref) => {
  ...
  const listProvince = useAppSelector(
    (state) => state.commonSlice.listProvince
  );
  const listDistrict = useAppSelector(
    (state) => state.commonSlice.listDistrict
  );
  ...
  return (
    <div className="filter-fast-expertise">
      <Card className="card-container">
        <ExpandSearch
          fields={fields.filter((el: any) => el.key !== 3 && el.key !== 4)}
          onSearch={onSearch}
          ref={expandRef}
        ></ExpandSearch>
      </Card>
