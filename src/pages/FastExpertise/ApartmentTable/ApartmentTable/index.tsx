...
const ApartmentTable = ({ filters }: any) => {
        ...
        render: (_: any, _record: any) => (
          <>
            <ListButtonActionUpdate
              editFunction={() => {
                navigate(
                  FAST_EXPERTISE_APARTMENT_EDIT.replace(":id", _record?.assetId)
                );
      ...
      <Card className="card-container" size="small">
        <div style={{ marginBottom: "4px" }}>
          <CardTitleCustomUpdate
            title="Danh sách định giá nhanh chung cư"
            addFunction={() => {
              dispatch(setType("create"));
              navigate(FAST_EXPERTISE_APARTMENT_CREATE);
            }}
            addButtonCode={BUTTON_CODES.dgn_chcc_them}
          />
        </div>
        <TableCustom
          ...
          onRow={(record, rowIndex) => {
            return {
              onDoubleClick: (event) => {
                navigate(
                  FAST_EXPERTISE_APARTMENT_VIEW.replace(":id", record?.assetId)
                );
                // handleOpenEditModal(_record);
              },
            };
