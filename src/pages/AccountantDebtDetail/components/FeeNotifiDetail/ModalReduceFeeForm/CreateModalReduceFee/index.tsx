...
import { newItem, formUploadReduceFee } from "./config";
...
const initialValue: FeeContentReduceFeeCreateType & {
  filesLength: number;
} = {
  ...newItem,
  filesLength: 0,
...
const CreateModalReduceFee: React.FC<Props> = ({
  index,
  isOpen,
  closeModal,
  addNew,
  ...
  const formCreateNewData = useFormik({
    ...
    validationSchema: formUploadReduceFee,
    validateOnChange: true,
    onSubmit: (data: any) => {
      if (index !== undefined) {
        handleUploadFileEcm(index, data);
      }
    },
  });
  ...
  const handleUploadFileEcm = (index: number, data: any) => {
    addNew({
      ...formCreateNewData.values,
      index,
      fileName: fileList[0],
      dateUploadReduceFee: data.dateUploadReduceFee,
    });
    handleCloseModal();
  };
  ...
  const inputs: InputFiledParams[] = [
    {
      key: 1,
      label: "Ngày upload tờ trình giảm phí",
      type: DATE_PICKER,
      showTime: true,
      ...
      value: formCreateNewData.values.dateUploadReduceFee
        ? dayjs(formCreateNewData.values.dateUploadReduceFee)
        : null,
      error: formCreateNewData.errors.dateUploadReduceFee,
      touched: formCreateNewData.touched.dateUploadReduceFee,
      onChange: (value: any) => {
        handleChangeValue({
          dateUploadReduceFee: value ? dayjs(value).toISOString() : null,
        });
      },
    },
  ];
...
export default CreateModalReduceFee;
