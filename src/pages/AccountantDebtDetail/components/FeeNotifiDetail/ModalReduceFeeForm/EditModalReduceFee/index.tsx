...
import { FeeContentReduceFee } from "constant/types/appraisalFilesDetail";
...
type Props = {
  ...
  isOpen: boolean;
  closeModal: () => void;
  dataInit: FeeContentReduceFee | undefined;
  handleEdit: (pr: any) => void;
};

const { TEXT_AREA, DATE_PICKER } = TYPE_FIELD;

const initialValue: FeeContentReduceFee & {
  filesLength: number;
} = {
  ...newItem,
  filesLength: 0,
...
const EditModalReduceFee: React.FC<Props> = ({
  index,
  isOpen,
  closeModal,
  handleEdit,
  ...
  const handleUploadFileEcm = async (index: number, data: any) => {
    if (isReplaceFile) {
      handleEdit({
        ...formEditData.values,
        index,
  ...
  const inputs: InputFiledParams[] = [
    {
      ...
      value: formEditData.values.dateUploadReduceFee
        ? dayjs(formEditData.values.dateUploadReduceFee)
        : null,
      error: formEditData.errors.dateUploadReduceFee,
      touched: formEditData.touched.dateUploadReduceFee,
      onChange: (value: any) => {
        handleChangeValue({
          dateUploadReduceFee: value
...
export default EditModalReduceFee;
