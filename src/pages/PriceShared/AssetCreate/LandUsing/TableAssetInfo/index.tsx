...
import {
  defaultItem,
  initialValue,
} from "pages/PriceShared/AssetCreate/LandUsing/config";
import {
  useAddressProvince,
  useDataSource,
  useSourceInfor,
...
type Props = {
  compareAssets: Array<ComparedAssetLandEstateCreateType>;
  ...
  const handleChangeDatasource = async (
    ...
    if (
      ...
      const resultConstructionPrice =
        ...
        Math.round((Number(newData[index]?.remainQuality) / 100));
  ...
  return (
    <Card size="small">
      <TableAddColEdit
        ...
        handleAddCol={isCvtd ? handleAddCol : undefined}
        handleCoppyCol={isCvtd ? handleCoppyCol : undefined}
        handleSubCol={isCvtd ? handleSubCol : undefined}
      />
    </Card>
  );
};
