...
import "pages/PriceShared/AssetCreate/Device/style.scss";
...
import ModalChangeAsset from "pages/PriceShared/AssetCreate/ModalChangeAsset";
...
const Device = () => {
  ...
  const [selectedAssetLevelThreeId, setSelectedAssetLevelThreeId] =
    useState<number>(ASSET_LV3.MACHINE);
  ...
  const dataAssetLevelThrees = useAssetLevelThree(ASSET_LV2.MACHINE);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [PRICE_SHARED_DEVICE_CREATE_ASSET]);
      ...
      <div className="page-container">
        <div className="header-wrapper">
          <Space className="title-wrapper" style={{ display: "flex" }} size={8}>
            <Typography className="title">Tạo tài sản MMTB</Typography>
          </Space>
          <Space
          ...
          >
            <Button className="btn-save" onClick={handleSubmit}
              disabled={isNotAllowed(currentPagePermissions, BUTTON_CODES.mmtb_gui_duyet)}
            >
              Gửi duyệt
            </Button>
          </Space>
