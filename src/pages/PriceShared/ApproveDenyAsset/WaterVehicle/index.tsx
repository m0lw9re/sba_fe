...
import "pages/PriceShared/ApproveDenyAsset/WaterVehicle/style.scss";
...
const WaterVehicle = () => {
  ...
  const { data: assetInfor, mutate } = useAssetStoredDetail(ASSET_PRICES_SHARED_TYPE.WATERWAY_VEHICLE, id || "");
  ...
  useEffect(() => {
    let breadCrumb = [
      ...
      },
      {
        ...
        path: PRICE_SHARED_APPROVE_WATERVEHICLE.replace(":id", id || ""),
      } 
    ];
    dispatch(setSelectedBeadCrumb(breadCrumb));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [PRICE_SHARED_APPROVE_WATERVEHICLE,typeAction]);
      ...
      <div className="page-container">
        <Space direction="vertical" style={{ width: "100%" }}>
          <Row justify={"space-between"}>
            <Typography className="title">{typeAction ? 'Tài sản PT đường thủy': 'Phê duyệt tài sản PT đường thủy'}</Typography>
          ...
          <StoredAssetWaterVehicleForm
            assetLevelThreeId={assetInfor?.data?.type || ASSET_LV3.SHIP}
            ref={btnSubmitForm} 
            formType={formType} 
            />
        </Space>
