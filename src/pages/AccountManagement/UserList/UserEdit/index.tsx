...
import "pages/AccountManagement/UserList/UserEdit/style.scss";
...
const { INPUT, DATE_PICKER, SELECT } = TYPE_FIELD;
...
const UserEdit: React.FC<Props> = ({ isOpen, closeModal, staffEdit }) => {
  ...
  const form = useFormik({
    initialValues: {} as any | StaffEditType,
    ...
    onSubmit: async (data: any) => {
      if (data) {
        try {
          ...
          // upload ảnh
          if (fileList.length > 0 && data.ecmId !== fileList[0]?.uid) {
          ...
          const response = await accountApi.updateStaff(convertedData);
          ...
          message.error(
            "Lỗi không xác định! Cập nhật thông tin người dùng không thành công!"
          );
        }
      }
    },
  ...
  useEffect(() => {
    form.setValues({
      ...staffEdit,
      daysOff:
        staffEdit && staffEdit.daysOff === "string"
          ? dayjs()
          : staffEdit && staffEdit.daysOff,
    });
  }, [staffEdit]);
  ...
  const inputs: InputFiledParams[] = [
    ...
    },
    {
      ...
      options:
        data?.map((item: PositionType) => {
          return {
            label: item.positionName.toUpperCase(),
    ...
    },
    {
      ...
      size: "small",
      value: form.values.daysOff ? dayjs(form.values.daysOff) : null,
      css: css,
      labelCol: labelCol,
      wrapperCol: wrapperCol,
  ...
  useEffect(() => {
    if (staffEdit?.ecmId) {
      const getAll = async () => {
        ...
        const item: AssetImageType = {
          ecmId: staffEdit.ecmId || "",
          filename: staffEdit.filename || "",
          mediaType: staffEdit.mediaType || "",
          whoUpload: staffEdit.whoUpload || "",
          dateUpload: staffEdit.dateUpload || "",
          assetImageId: 0,
          appraisalFileId: "",
          description: "",
          type: 0,
    ...
    >
      <Space direction="vertical" style={{ width: "100%" }} size={16}>
        <Row
          ...
          align={"middle"}
        >
          <CardTitleCustomUpdate title="Cập nhật tài khoản" />
          <Button
            size="small"
            shape="circle"
            icon={<Icons.close />}
