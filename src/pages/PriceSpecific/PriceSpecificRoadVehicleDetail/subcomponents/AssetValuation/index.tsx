...
import {
  defaultColumns,
} from "pages/PriceSpecific/PriceSpecificRoadVehicleDetail/subcomponents/AssetValuation/config";
...
const AssetValuation: React.FC<Props> = ({ data }) => {
  ...
  useEffect(() => {
    ...
    if (data !== null && data !== undefined) {
      ...
      setTotal({
        totalValueApprovaled: data.body?.tableKQCTXD?.reduce((sum: any, item: any) => sum + (item.totalValueApprovaled || 0), 0),
        ...
        <Space size={4} style={{ width: "100%" }} direction="vertical">
          ...
          <Row>
            ...
            >
              ...
              >
                <Typography.Text strong>{numberUtils.formatNumber(total.totalValueApprovaled)}</Typography.Text>
              </Form.Item>
              <Form.Item
                labelCol={labelCol}
                wrapperCol={wrapperCol}
