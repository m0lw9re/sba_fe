...
const PriceSpecificTable: React.FC<Props> = ({ filters, setFilters }) => {
  ...
  const handleNavigatePriceSpecificDetail = (id: string) => {
    ...
    navigate(PRICE_SPECIFIC_APPARTMENT.replace(":id", id), {
      state: {
        label: "BĐS",
    ...
    if (col.key === 2) {
      return {
        ...
        render: (assetCode: any, record: any) => (
          <>
            <Link
              to={PRICE_SPECIFIC_APPARTMENT_DETAIL.replace(
                ":id",
                record.assetId
              )}
              onClick={() => {
          ...
          <>
            <ListButtonActionUpdate
              ...
              viewFunction={() => {
                record?.assetId &&
                  navigate(
                    PRICE_SPECIFIC_APPARTMENT_DETAIL.replace(
                      ":id",
                      record.assetId
                    ),
                    {
  ...
  const exportExcell = async () => {
    try {
      ...
      saveAs(blob, "Can_ho_chung_cu.xlsx");
    } catch (error: any) {
          ...
          <Col>
            ...
            >
              <ButtonCustom
                ...
                onClick={showConfirm}
                disabled={exporting}
                code={BUTTON_CODES.kgchcc_xuat_du_lieu}
              />
            </Space>
          </Col>
        </div>
