...
const PriceSpecificTable: React.FC<Props> = ({ filters, setFilters }) => {
  ...
  // const { data, isLoading, error } = usePriceAssets(params, filters)
  ...
  const handleNavigatePriceSpecificDetail = (id: string) => {
    ...
    navigate(PRICE_SPECIFIC_USING_LAND_DETAIL.replace(":id", id), {
      state: {
        label: "BĐS",
    ...
    if (col.key === 2) {
      return {
        ...
        render: (assetCode: any, record: any) => (
          <>
            <Link
              to={PRICE_SPECIFIC_USING_LAND_DETAIL.replace(
                ":id",
                record.assetId
              )}
              onClick={() => {
          ...
          <>
            <ListButtonActionUpdate
              ...
              viewFunction={() => {
                record?.assetId &&
                  navigate(
                    PRICE_SPECIFIC_USING_LAND_DETAIL.replace(
                      ":id",
                      record.assetId
                    ),
                    {
          ...
          <Col>
            ...
            >
              <ButtonCustom
                ...
                onClick={showConfirm}
                disabled={exporting}
                code={BUTTON_CODES.kgbds_xuat_du_lieu}
              />
            </Space>
          </Col>
        </div>
