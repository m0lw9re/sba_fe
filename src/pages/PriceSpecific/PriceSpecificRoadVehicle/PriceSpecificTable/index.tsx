...
import { Card, Col, message, Modal, Space, Spin, Typography } from "antd";
...
const PriceSpecificTable: React.FC<Props> = ({ filters, setFilters }) => {
          ...
          <>
            <ListButtonActionUpdate
              ...
              viewFunction={() => {
                record?.assetId &&
                  navigate(
                    PRICE_SPECIFIC_ROAD_VEHICLE_DETAIL.replace(
                      ":id",
                      record.assetId
                    ),
                    {
  ...
  const exportExcell = async () => {
    try {
      ...
      saveAs(blob, "Phuong_tien_duong_bo.xlsx");
    } catch (error) {
          ...
          <Col>
            ...
            >
              <ButtonCustom
                ...
                onClick={showConfirm}
                disabled={exporting}
                code={BUTTON_CODES.kgptdb_xuat_du_lieu}
              />
            </Space>
          </Col>
        </div>
