...
import {
  defaultColumns,
} from "pages/PriceSpecific/PriceSpecificAppartmentDetail/subcomponents/AssetValuation/config";
...
const AssetValuation: React.FC<Props> = ({ data }) => {
  ...
  useEffect(() => {
    ...
    if (data !== null && data !== undefined) {
      data.body.tableKQ.forEach((record: any, index: number) => {
          const assetName = data.body.tablePP[index]?.assetName || data.body.tablePP[0]?.assetName;
          const assetChildName = data.body.tablePP[index]?.assetChildName || data.body.tablePP[0]?.assetChildName;
