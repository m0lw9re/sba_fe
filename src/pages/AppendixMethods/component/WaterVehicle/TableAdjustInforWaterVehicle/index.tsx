...
import AddAdjustCriteriaModal from "pages/AppendixMethods/component/WaterVehicle/TableAdjustInforWaterVehicle/component/AddAdjustCriteriaModal";
...
const TableAdjustInforWaterVehicle: FC<Props> = ({
  adjustTable,
  storedAssets,
  ...
  const renderSummaryDataTable = () => {
    ...
    const objUnitPrice = {
      key: 4,
      stt: "",
      adjustContent: "Đơn giá cuối cùng (đồng)",
  ...
  const CalSummaryTable = () => {
    ...
    localAdjustTable.forEach((el: any, index) => {
      ...
      const estimatePrice = foundStoredAsset
        ? foundStoredAsset.estimatePrice
        : 0;

      const PriceChenhLech = Math.round(
        (totalRatios[index] * estimatePrice) / 100
      );

      const PriceDanChieu = Math.round(estimatePrice + PriceChenhLech);
...
export default TableAdjustInforWaterVehicle;
