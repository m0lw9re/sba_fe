    ...
    .of(
      Yup.object().shape({
        constructionTypeId: Yup.string()
          ...
          .typeError("Vui lòng chọn loại công trình xây dựng"),
        constructionNameId: Yup.string()
          .required("Vui lòng chọn mô tả đặc tính kỹ thuật")
          .typeError("Vui lòng chọn mô tả đặc tính kỹ thuật"),
        constructionArea: Yup.number()
          .required("Vui lòng nhập diện tích sử dụng")
          .typeError("Vui lòng nhập diện tích sử dụng")
          .test("constructionAreaFormSchema", "Chỉ nhập được 50 ký tự", (val) =>
            validLengthInput(val, 50)
          ),
        constructionLegalTypeId: Yup.string()
          .required("Vui lòng chọn hồ sơ pháp lý")
          .typeError("Vui lòng chọn hồ sơ pháp lý"),
        floors: Yup.number()
          .nullable()
          .test("floorsFormSchema", "Chỉ nhập được 4 ký tự", (val) =>
            validLengthInput(val, 4)
          ),
        baseFloors: Yup.number()
          .nullable()
          .test("baseFloorsFormSchema", "Chỉ nhập được 4 ký tự", (val) =>
            validLengthInput(val, 4)
          ),
        ...
        repairYear: Yup.number()
          .nullable()
          .test("repairYearFormSchema", "Chỉ nhập được 4 ký tự", (val) =>
            validLengthInput(val, 4)
          ),
        remainingQuality: Yup.number()
          .nullable()
          .min(0, "Không thể nhập giá trị nhỏ hơn 0")
          .max(100, "Không thể nhập giá trị lớn hơn 100")
          .required("Vui lòng nhập CLCL")
          .test("repairYearFormSchema", "Chỉ nhập được 3 ký tự", (val) =>
        ...
        describe: Yup.string()
          .nullable()
          .test("describeFormSchema", "Chỉ nhập được 1000 ký tự", (val) =>
            validLengthInput(val, 1000)
          ),
          furnitures: Yup.string()
          .nullable()
          .required("Vui lòng nhập nội thất")
          .test("furnituresFormSchema", "Chỉ nhập được 1000 ký tự", (val) =>
            validLengthInput(val, 1000)
          ),
      })
    )
...
const Contruction = forwardRef<RefProps, Props>(
  ({ visible, setVisible, contructionLandInfor, assetLandInforId }, ref) => {
    ...
    const columns = defaultColumns.map((column) => {
      ...
      } else if (column.key === 11) {
        return {
          ...column,
          error: form.errors.constructions,
          touched: form.touched.constructions,
        };
      } else if (column.key === 12) {
        return {
          ...column,
          error: form.errors.constructions,
          touched: form.touched.constructions,
        };
      } else if (column.key === 6) {
        return {
          ...column,
          error: form.errors.constructions,
          touched: form.touched.constructions,
        };
      } else if (column.key === 7) {
        return {
          ...column,
          error: form.errors.constructions,
          touched: form.touched.constructions,
        };
      } else if (column.key === 8) {
        return {
          ...column,
          error: form.errors.constructions,
          touched: form.touched.constructions,
        };
      } else if (column.key === 9) {
        return {
          ...column,
          error: form.errors.constructions,
          touched: form.touched.constructions,
        };
      } else if (column.key === 10) {
        return {
          ...column,
          error: form.errors.constructions,
          touched: form.touched.constructions,
        };
      } else if (column.key === 13) {
        return {
          ...column,
          error: form.errors.constructions,
          touched: form.touched.constructions,
    ...
    useEffect(() => {
      if (contructionLandInfor && constructionOptions) {
