...
import { useReportsDetailWeek } from "utils/request/useReportDetailWeek";
...
type Props = {
  ...
  setFilters: (filters: FilterReportDetailWeek) => void;
};

const TableReportDetailWeek: React.FC<Props> = ({ filters, setFilters }) => {
  ...
  const exportExcell = async () => {
    try {
      ...
      saveAs(blob, "Hs_bao_cao_chi_tiet_tuan.xlsx");
    } catch (error) {
      // message.error("Xuất báo cáo thất bại!");
      message.error(`Không tìm thấy báo cáo HS chi tiết tuần!`);
    } finally {
      setExporting(false);
    }
  };
  ...
  const showConfirm = () => {
    Modal.confirm({
      title: "Xác nhận",
      content: `Bạn có chắc muốn xuất báo cáo hồ sơ chi tiết tuần`,
      okText: "Xác nhận",
      cancelText: "Hủy",
      onOk: () => {
        exportExcell();
  ...
  const { data, isLoading, error, mutate } = useReportsDetailWeek(params, {
    ...filters,
    startDate: filters.startDate
      ? dayjs(filters?.startDate).format("DD-MM-YYYY 00:00:00")
      : null,
...
export default TableReportDetailWeek;
