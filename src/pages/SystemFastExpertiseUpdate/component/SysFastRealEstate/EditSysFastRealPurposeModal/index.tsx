...
type Props = {
  isOpenModal: boolean;
  closeModal: () => void;
  realEstatePurpose: SystemFastExpertiseRealEstate;
...
const EditSysFastRealPurposeModal: React.FC<Props> = ({
  ...
  const form = useFormik({
    initialValues: {
      ...
      type: realEstatePurpose.type,
      typeName: realEstatePurpose.typeName,
      rateId: realEstatePurpose.rateId,
      rateName: realEstatePurpose.rateName,
      ratePerMain: realEstatePurpose.ratePerMain,
  ...
  console.log(usingPurposeOptions, form.values.rateId);
  ...
  const inputFields: InputFiledParams[] = [
    ...
    {
      key: 3,
      label: "Tỉ lệ so với Đất ở tại đô thị",
      placeholder: "Nhập",
      type: INPUT_NUMBER,
      require: true,
      ...
      labelCol: { xs: 6, sm: 6, md: 6, lg: 10, xl: 10 },
      wrapperCol: { xs: 18, sm: 18, md: 18, lg: 14, xl: 14 },
      css: css,
      ...
      >
        ...
        >
          <CardTitleCustomUpdate
            title={
              modalType === "add"
                ? "Thêm cấu hình mục đích sử dụng"
                : "Sửa cấu hình mục đích sử dụng"
            }
            color="#FFF"
          />
          <Button
