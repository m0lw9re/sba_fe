...
const SysFastRealPurposeTable = () => {
  ...
  const handleEdit = (id: number, typeName?: string, description?: string) => {
    setOpenAddModal(true);
    setModalType("edit");
  ...
  const { data, error, isLoading, mutate } = useSystemFast(params, {
    type: 4,
    ...
    },
    {
      ...
      render: (typeName) => (
        <Tooltip title={typeName}>
          <div className="inline-text">{typeName}</div>
        </Tooltip>
      ),
    },
      ...
      render: (_, record: any) => (
        <>
          <ListButtonActionUpdate
            editFunction={() =>
              handleEdit(record.id, record.typeName, record.description)
            }
  ...
  return (
    <div className="categories-administratives-table-container">
      <Card className="card-container" size="small">
        ...
        >
          <CardTitleCustomUpdate title="Cấu hình nhóm tầng" />
          <ButtonCustom
            label="Thêm"
            type="primary"
